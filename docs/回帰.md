...
## 機械学習において注意すること
* 入力変数同士が高い相関関係を持つと思うような学習結果を得られない可能性がある(多重共線性: 気温で例を上げると摂氏と華氏を同時に学習させるなど)
...
## 回帰について
教師あり学習は何らかの値を予測するモデルを作っており、学習することでデータの`特徴`を掴む。

回帰は`数値`を予測する(分類はカテゴリを予測)。

回帰分析とは、ある変数を用いて他の変数を予測(予測)するモデルを作ること。

例えば、アイスコーヒーを例に上げる。

`(アイスコーヒーの売上高) = (ある定数)+(ある係数)*(気温)`

* 目的変数(t) --- アイスコーヒーの売上高
* 説明変数(x) --- 気温

Y軸にt、X軸にxをプロットすることで線形モデルを作ることができる(最小二乗法)。
...
機械学習の場合、Label EncodingよりもOne-Hot Encodingの方が有効。

`y = w1x1 + w2x2 + ・・・ + wMxM +b`

Label Encodingだと0, 1, 2と上から順にラベルを振っていくことになるが、この数が多いと
掛け合わせるxのパラメータに大きな乖離が生まれる。
...
上記だとA型に対して、AB型の値が3倍以上になり掛け合わせるパラメータに不整合が生じる。
...
## スパースな状態
スパース(疎)とは、大量の必要ない説明変数が存在している状態を指す。
